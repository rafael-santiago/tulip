...
by ``Stan Getz`` of a famous ``Brazilian song`` called ``O Pato`` (a.k.a ``The duck``). Here goes my lousy ``Stan Getz`` solo's
adaptation for the guitar. Separated into ``Theme`` and ``Solo``:
