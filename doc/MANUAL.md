...
the output production in other words the ``processors``. Here you can find something about my motivations on writing this
software. If you are in a rush try to go straight to the section "The Tulip system".

----

## My motivations

I have been playing (or at least trying) guitar since my twelve years old, during this years I had only two teachers.
...
It is truly that this kind of notation is quite old. In my formal studies with my first teacher I used a book from ``1966``
written by a man called ``Fernando Azevedo``. This book was common between people here called "seresteiros",
a kind of bohemians signers which produced a significant collection of Brasilians popular Songs, however, nowadays
everyone tends to take it as a little bit old fashioned, specially the harmonies. Even so I could learned important
things related with the basics of the guitar playing from this *papyrus* and even today I use these learnings.
...
This notation is the main notation behind ``Tulip``'s ``DSL`` so if you want to master ``Tulip`` you must master it before.
...
However in the fret number case we have some particularities. Some people when hits the fret number ten just numbered it as
...
Maybe the usage of ``00`` for fret ten can confuse you at the beginning but with practice your brain will start to think
in this way with any difficulty.

Beware that if you search the term ``Numeric cipher`` you will reach subjects dealing with ``Cryptography``.
I am saying it because I am hooked on this too but here I am translating literally from Portuguese what means
``Cifra Numérica``. What in this case deals with ``Music`` here in my country.

### "The paper stack blues"

I like to transcribe/adapt some songs as exercise and for fun issues too, of course. I am in this "bussiness" since my
teenagerhood. As a result during these years I produced huge stacks of paper. One of my motivations to create ``Tulip``
(still in ``2005``) was a effort in order to stop with this mess. The thurth is that I did not stop to push papers to
...
``Tulip`` is a idea/software which can be splitted and understood in two main parts:
